//
//  JJAddNewReceiveAddressViewController.m
//  XiaoHaiTun
//
...
@interface JJAddNewReceiveAddressViewController ()
...
@implementation JJAddNewReceiveAddressViewController
...
- (void)initBaseView {
    ...
    [lineView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.bottom.equalTo(self.identityCartView);
    ...
    [self.setDefaultAddressBtn setTitle:@"设置默认按钮" forState:UIControlStateNormal];
...
- (void)save:(UIButton *)btn{
    ...
    NSDictionary *params = @{@"shipping_user" : shipping_user, @"mobile" : mobile, @"province" : province, @"city" : city ,@"area" : area, @"id_card" : id_card, @"address" : address, @"is_default" : is_default};
    
    
    // 发送添加收货地址请求
    NSString * ThemeRequesturl = [NSString stringWithFormat:@"%@%@",DEVELOP_BASE_URL, API_UPDATE_SHIPPING_ADDRESS];
    
    [HFNetWork postWithURL:ThemeRequesturl params:params success:^(id response) {
        ...
        [MBProgressHUD showHUDWithDuration:1.0 information:@"地址添加成功" hudMode:MBProgressHUDModeText];
