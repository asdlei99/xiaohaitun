...
@interface JJMyCollectionGoodsViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation JJMyCollectionGoodsViewController
...
//            wealSelf.tableView.ttipTryOnceView = [JJTipTryOnceView tipTryOnceView];
...
- (void)requestCollectionQuery {
    ...
    NSDictionary * params = @{@"user_id" : [User getUserInformation].userId , @"is_activity" : @0};
...
//    NSDictionary *params = @{@"user_id" : [User getUserInformation].userId, @"item_id" : model.goodsID , @"action_id" : @2 , @"is_activity" : @0};
...
- (void)requestDeleteCollect {
    ...
    NSDictionary *params = @{@"user_id" : [User getUserInformation].userId, @"item_list" : [Util arrayToJson:deleteArray]};
    [HFNetWork postWithURL:deleteCollectRequesturl params:params success:^(id response) {
        [MBProgressHUD hideHUD];
        if (![response isKindOfClass:[NSDictionary class]]) {
            return ;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    else
    {
        ...
        //    goodsDetailViewController.is_collect = YES;
