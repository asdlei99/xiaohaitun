...
- (void)keyboardNotify:(NSNotification *)notify{
    ...
    if ([notify.name isEqualToString:UIKeyboardWillShowNotification]) {//键盘显示
        ...
        //        DebugLog(@"%@    %@",NSStringFromCGRect(tempView.frame),NSStringFromCGRect(tempView.superview.frame));
        CGRect rect = [tempView convertRect:tempView.bounds toView:window];//计算响应者到和屏幕的绝对位置
        ...
        if (tempHeight > keyboardY) {
            ...
            if (APPWINDOWHEIGHT-tempHeight < 0) {//判断是否超出了屏幕,超出屏幕做偏移纠正
                ...
                CGPoint point = rect.origin;
                offsetY = keyboardY - tempHeight + point.y;
            }
            NSLog(@"%lf   %lf   %lf",keyboardY,tempHeight,offsetY);
            if (duration > 0) {
